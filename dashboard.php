...
<script>
    ...
    function renderIerbMonitor() {
        ...
        monitorRecords.forEach(record => {
            ...
            row.innerHTML = `
                <td><strong>${escapeMonitorHtml(record.groupId || record.studentId)}</strong></td>
                ...
                <td><span class="stage-tag">${escapeMonitorHtml(record.stage || 'Stage 1')}</span></td>
                <td>${escapeMonitorHtml(record.requirements || 'None')}</td>
                <td><span class="progress-value">${escapeMonitorHtml(record.progress || '0')}%</span></td>
                <td><span class="status-badge ${statusClass}">${escapeMonitorHtml(record.status || 'Pending')}</span></td>
                <td><button class="icon-btn" data-monitor-action="remind" title="Send follow-up email"><i class="fa-solid fa-paper-plane"></i></button><button class="icon-btn" data-monitor-action="summary" title="AI summary" data-id="${record.id}"><i class="fa-solid fa-file-lines"></i></button></td>`;
    ...
    (async function loadRepositoryPreview() {
        try {
            ...
            docs.forEach(d => {
                ...
                row.innerHTML = `<i class="fa-regular fa-file-lines"></i><span>${escapeMonitorHtml(d.originalName)}</span><small>${escapeMonitorHtml(d.student || 'Unassigned')} &bull; ${escapeMonitorHtml(d.reviewStatus)}</small>`;
